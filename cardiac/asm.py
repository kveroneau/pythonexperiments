...
class Assembler(Cmd):
    ...
    def configure(self):
        ...
        self.var_cards = [] #: A list of variable cards for the deck.
    ...
    def write_addr(self):
        ...
        if self.addr:
            addr = self.padding+str(self.addr)
            self.write_cards('0%s' % addr[-2:])
            self.addr +=1
        self.size +=1
    ...
    def do_end(self, args):
        """ Finalizes your code. """
        for card in self.var_cards:
            ptr = self.padding+str(self.ptr)
            self.write_addr()
            if not card[1].startswith('*'):
                self.write_cards(card[1][-3:])
                self.labels[card[0]] = ptr[-2:]
            else:
                self.write_cards('1%s' % card[1])
                self.labels[card[0]] = ptr[-2:]
        if self.start:
            self.write_cards('002', '%s' % self.start)
        self.buffer.seek(0)
        buf = StringIO()
        for card in self.buffer.readlines():
            ...
            buf.write(card)
        buf.seek(0)
        self.stdout.write(''.join(buf.readlines()))
        return True
    def do_var(self, args):
        ...
        if s[2].startswith('*'):
            self.var_cards.append((s[0], s[2]))
        else:
            value = int(s[2])
            self.var_cards.append((s[0], '000'+str(value)))
    ...
    def default(self, line):
        ...
        if op in self.op_map:
            if self.pc == self.start:
                self.start = '%s%s' % (self.op_map[op], arg)
            else:
                self.write_addr()
